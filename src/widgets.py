...
class Button(Widget):
    ...
    def __init__(
        ...
        surface: pygame.surface.Surface or None = None,
    ) -> None:
        if surface:
            self.screen = surface 
        self.text = text
        ...
        self.text_surf = font.render(text, True, foreground_color)
    ...
    def draw(self):
        ...
        clicked = self.rect.collidepoint(pos) and pygame.mouse.get_pressed()[0]

        # Draw button
        pygame.draw.rect(self.screen, self.background_color, self.rect)

        if self.outline: 
            pygame.draw.rect(self.screen, BLACK, self.rect, width=True)
